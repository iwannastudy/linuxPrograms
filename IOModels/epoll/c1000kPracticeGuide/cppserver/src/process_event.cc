                       ...
                       TIMER *timerlink) 
{
    ...
    if ((!gFDITEM[fd]->m_activeclose) && (evlist->events & EPOLLIN))
    {
        ...
        {
            if (0 > gFDITEM[fd]->m_readfun(epollfd, fd, timerlink))
            {
    ...
    if ((!gFDITEM[fd]->m_activeclose) && (evlist->events & EPOLLOUT)) 
    {
        ...
        {
            if (0 > gFDITEM[fd]->m_writefun(epollfd, fd, timerlink))
            {
                gFDITEM[fd]->m_activeclose = true;
            }
        }
    ...
    {
        timerlink->remove_timer(&(gFDITEM[fd]));
        gFDITEM[fd]->m_closefun(epollfd, fd, timerlink);
    }
}
